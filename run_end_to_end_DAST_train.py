...
from utils.blf_model_evaluation.test_models_on_given_data import test_models
from utils.evaluation_metrics.jaccard import jaccard_similarity_coefficient
from models.DAST.load_context_transformation.generate_data_using_GAN import generate_synthetic_data_using_GAN, \
    generate_synthetic_temperature
...
if __name__ == '__main__':
    synthetic_train_x, synthetic_train_y = [], []
    ...
    for target_context in all_context_set:
        ...
        if target_context in given_context_set:
            ...
            df_synthetic_in_this_context = combine_load_traces_with_augmented_remainder(df_synthetic_in_this_context=df_synthetic_in_this_context,
                                                                                        cl_model=cl_model)
            # get X and Y from df_synthetic
            synthetic_X, synthetic_Y = [], []
            for i in range(df_synthetic_in_this_context.shape[0] - 2 * seq_length):
                synthetic_X.append(np.array(df_synthetic_in_this_context.iloc[i: i + seq_length, 6:-2]))
                synthetic_Y.append(np.array(df_synthetic_in_this_context.loc[i + seq_length: i + 2 * seq_length - 1, 'nor_el']))
            synthetic_X, synthetic_Y = np.array(synthetic_X), np.array(synthetic_Y)

            synthetic_train_x.append(synthetic_X)
            synthetic_train_y.append(synthetic_Y)
            print()
        else:
    ...
    synthetic_train_x = np.concatenate(synthetic_train_x, axis=0)
    synthetic_train_y = np.concatenate(synthetic_train_y, axis=0)
    print('    synthetic X shape: {}'.format(synthetic_train_x.shape))
    print('    synthetic Y shape: {}'.format(synthetic_train_y.shape))

    # train BLF model
    print('train BLF model')
    synthetic_data_model_name = '{}_dast'.format(chosen_model_structure)
    train_model(data_x=synthetic_train_x, data_y=synthetic_train_y, model_name=synthetic_data_model_name,
                input_dim=input_dim, hidden_dim=hidden_dim, num_layers=num_layers, batch_size=batch_size,
                learning_rate=learning_rate, num_of_BLF_training_epochs=num_of_BLF_training_epochs,
                sparse_output_dim=sparse_output_dim, output_dim=output_dim, enc_hid_dim=enc_hid_dim, dec_hid_dim=dec_hid_dim)

    # evaluate BLF model
    print('evaluate BLF model')
    given_context = list(given_context_set)[0]

    with open('./tmp_data/building_tmp_data/{}_season_{}_{}_data_dict.pkl'.format(target_building_name, given_context, given_data_length), 'rb') as r:
        data_dict = pickle.load(r)
    df = data_dict.get('df')
    load_max = data_dict.get('load_max')
    load_min = data_dict.get('load_min')
    ...
    for i in range(df.shape[0] - 2 * seq_length):
        original_test_x.append(np.array(df.iloc[i: i + seq_length, 6:-2]))
        original_test_y.append(np.array(df.loc[i + seq_length: i + 2 * seq_length - 1, 'nor_el']))
    original_test_x = np.array(original_test_x)
    original_test_y = np.array(original_test_y)

    print('    synthetic train X shape: {}'.format(synthetic_train_x.shape))
    print('    synthetic train Y shape: {}'.format(synthetic_train_y.shape))
    print('    original test X shape: {}'.format(original_test_x.shape))
    print('    original test Y shape: {}'.format(original_test_y.shape))

    test_model(data_x=original_test_x, data_y=original_test_y, load_max=load_max, load_min=load_min,
               model_name=synthetic_data_model_name, input_dim=input_dim, hidden_dim=hidden_dim, num_layers=num_layers,
               batch_size=batch_size, sparse_output_dim=sparse_output_dim, output_dim=output_dim,
