...
class SAX(object):
    ...
    a series of raw_data to a string, which can then be compared with other
    ...
    def to_letter_rep(self, x):
        """
        Function takes a series of raw_data, x, and transforms it to a string representation
        """
        x = self.normalize(x)
    ...
    def to_PAA(self, x):
        """
        Function performs Piecewise Aggregate Approximation on raw_data set, reducing
        the dimension of the dataset x to w discrete levels. returns the reduced
        dimension raw_data set, as well as the indices corresponding to the original
        raw_data for each reduced dimension
        """
        n = len(x)
    ...
    def compare_letters(self, la, lb):
        ...
        """
        return self.compareDict[la+lb] * 2 + self.compareDict[la+lb] ** 2
...
if __name__ == '__main__':
    ...
    sax = SAX(wordSize=8, alphabetSize=8)
    st1, _ = sax.to_letter_rep(ts1)
    st2, _ = sax.to_letter_rep(ts2)
    dist = sax.compare_strings(st1, st2)
    print('st1', st1)
    print('st2', st2)
    print(dist)