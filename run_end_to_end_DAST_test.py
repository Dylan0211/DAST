...
if __name__ == '__main__':
    ...
    for target_context in all_context_set:
        ...
        if target_context in given_context_set:
            ...
            cl_sample_a, cl_sample_b, cl_label = generate_positive_and_negative_samples(
            ...
            df_synthetic_in_this_context = combine_load_traces_with_augmented_remainder(
                df_synthetic_in_this_context=df_synthetic_in_this_context,
                cl_model=cl_model)
            ...
            for i in range(df_synthetic_in_this_context.shape[0] - 2 * seq_length):
                ...
                synthetic_Y.append(
                    np.array(df_synthetic_in_this_context.loc[i + seq_length: i + 2 * seq_length - 1, 'nor_el']))
    ...
    for model_name in model_name_list:
        if target_building_name in model_name:
            model_name_list.remove(model_name)

    model_rank_list_of_dast = test_models(data_x=synthetic_train_x, data_y=synthetic_train_y,
                                          load_max=load_max, load_min=load_min, input_dim=input_dim,
    ...
    for i in range(df.shape[0] - 2 * seq_length):
        test_x.append(np.array(df.iloc[i: i + seq_length, 6:-2]))
        test_y.append(np.array(df.loc[i + seq_length: i + 2 * seq_length - 1, 'nor_el']))
    test_x = np.array(test_x)
    test_y = np.array(test_y)

    model_rank_list_of_gt = test_models(data_x=test_x, data_y=test_y,
                                        load_max=load_max, load_min=load_min, input_dim=input_dim,
                                        hidden_dim=hidden_dim,
                                        num_layers=num_layers, batch_size=batch_size,
                                        sparse_output_dim=sparse_output_dim,
    ...
    for k in range(1, 32):
        jaccard_coefficient = jaccard_similarity_coefficient(rank_1=model_rank_list_of_dast,
                                                             rank_2=model_rank_list_of_gt,
                                                             k=k)
        print('    k = {}, jaccard coefficient = {}'.format(k, jaccard_coefficient))
